...
(function () {
  ...
  const closeOverlays = () => {
    ...
    document.querySelectorAll(".dfa17-overlay, .dl15-overlay").forEach(o => o.remove());
  };
  ...
  const fileDrawer = async (file, categoryId) => {
    ...
    const renderCards = () => {
      cardsRoot.innerHTML = pendingUploads.map((e, i) => `
        <div class="dfa17-card-row ${e.status}">
  ...
  if (!document.getElementById("dl10-css")) {
    ...
    css.textContent = `
      ...
      .dfa17-overlay{position:fixed;inset:0;z-index:85;display:flex;align-items:center;justify-content:center;padding:16px;overflow-y:auto;isolation:isolate}
      .dfa17-backdrop{position:fixed;inset:0;background:rgba(11,18,38,.18);backdrop-filter:none;-webkit-backdrop-filter:none;z-index:0;animation:dl10-fade .24s cubic-bezier(.23,1,.32,1)}
      .dfa17-card{position:relative;z-index:1;width:100%;max-width:520px;background:#fff;border-radius:20px;box-shadow:0 24px 60px rgba(11,18,38,.25);display:flex;flex-direction:column;max-height:min(88vh,780px);overflow:hidden}
