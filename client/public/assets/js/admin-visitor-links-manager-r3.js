(() => {
  ...
  const row = link => {
    ...
    return `<article class="vl-card" data-vl-id="${link.id}">
      ...
      <div class="vl-url"><code dir="ltr">${esc(url)}</code><button type="button" class="vl-icon-button" data-vl-action="copy" data-id="${link.id}" aria-label="نسخ الرابط">⧉</button><button type="button" class="vl-icon-button vl-whatsapp-button" data-vl-action="whatsapp" data-id="${link.id}" aria-label="مشاركة الرابط عبر واتساب" title="مشاركة عبر واتساب">واتساب</button><button type="button" class="vl-icon-button vl-telegram-button" data-vl-action="telegram" data-id="${link.id}" aria-label="مشاركة الرابط عبر تيليجرام" title="مشاركة عبر تيليجرام">تيليجرام</button></div>
  ...
  const createdNotice = () => {
    ...
    return `<aside class="vl-created-notice" role="status" aria-live="polite"><div><span class="vl-created-notice-mark">✓</span><div><strong>تم إنشاء الرابط والتحقق منه</strong><p>رابط «${esc(state.created.name || "الزائر")}» جاهز للمشاركة الآن.</p><code dir="ltr">${esc(url)}</code></div></div><div class="vl-created-notice-actions"><button type="button" class="btn" data-vl-action="copy-created">⧉ نسخ رابط الزائر</button><button type="button" class="btn btn-outline" data-vl-action="whatsapp-created">مشاركة عبر واتساب</button><button type="button" class="btn btn-outline vl-telegram-created" data-vl-action="telegram-created">مشاركة عبر تيليجرام</button><button type="button" class="vl-created-dismiss" data-vl-action="dismiss-created" aria-label="إغلاق رسالة الرابط المنشأ">×</button></div></aside>`;
  };
  const workspace = () => {
    const links = current();
  ...
  const shareTelegram = link => {
    const value = publicUrl(link.token);
    const message = "مرحباً، هذا رابط الزائر:";
    const url = `https://t.me/share/url?url=${encodeURIComponent(value)}&text=${encodeURIComponent(message)}`;
    window.open(url, "_blank", "noopener,noreferrer");
    toast("تم فتح تيليجرام لمشاركة رابط الزائر.");
  };
  const preview = link => {
    const url = publicUrl(link.token);
    const modal = document.createElement("div");
  ...
  document.addEventListener("click", async event => {
    ...
    try {
      ...
      else if (action === "telegram" && link) shareTelegram(link);
      else if (action === "telegram-created" && state.created) shareTelegram(state.created);
