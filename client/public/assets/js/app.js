...
function applyManagedContent(content) {
  ...
  if (Array.isArray(content.services)) {
    ...
    SERVICES = services.map((service, id) => ({ ...service, id, emoji: service.emoji || "📚", title: service.title || "خدمة", items: visible(service.items).sort((a, b) => Number(a?.sortOrder || 0) - Number(b?.sortOrder || 0)).map(item => typeof item === "string" ? { title: item, emoji: "•", buttonText: "اطلب الخدمة", buttonLink: "whatsapp" } : item) }));
...
function servicesPage() {
  ...
  if (!active) return `<div class="container section services-page-managed" data-services-layout="${esc(servicesStyle.layout || "grid")}" style="--services-section-bg:${safeCss(servicesStyle.background || "transparent")};--services-columns:${Math.max(1, Math.min(4, Number(servicesStyle.columns || 3)))}"><div class="text-center"><h1 class="page-title">${esc(servicesStyle.title || "خدماتنا الأكاديمية")}</h1><p class="page-intro">${esc(servicesStyle.description || "اختر القسم المطلوب لعرض الخدمات الفرعية المتاحة")}</p></div><div class="grid service-grid service-managed-grid">${SERVICES.map((srv) => `<button class="service-card service-template-${esc(srv.template || servicesStyle.template || "elevated")}" style="--service-accent:${safeCss(srv.color || "#4966d6")};--service-bg:${safeCss(srv.backgroundColor || "#fff")};--service-border:${safeCss(srv.borderColor || "#e4eaf5")}" data-action="select-service" data-service="${srv.id}">${srv.imageUrl ? `<img class="service-card-image" src="${esc(srv.imageUrl)}" alt="" />` : ""}<span class="service-emoji">${srv.iconUrl ? `<img src="${esc(srv.iconUrl)}" alt="" />` : esc(srv.emoji)}</span><h3>${esc(srv.title)}</h3>${srv.description ? `<p>${esc(srv.description)}</p>` : ""}</button>`).join("")}</div></div>`;
  return `<div class="container section"><button class="btn btn-outline" data-action="back-services">← العودة للأقسام</button><div class="service-detail-head"><span class="emoji">${active.iconUrl ? `<img src="${esc(active.iconUrl)}" alt="" />` : esc(active.emoji)}</span><div><h2>${esc(active.title)}</h2><p class="text-muted">${esc(active.description || "اختر الخدمة المطلوبة واضغط زر الطلب للتواصل الفوري")}</p></div></div><div class="grid">${active.items.map((item, i) => { const title = item.title || item; const href = item.buttonLink && item.buttonLink !== "whatsapp" ? safeHref(item.buttonLink) : wa(`أريد طلب خدمة: ${title} (من قسم: ${active.title})`); const external = !item.buttonLink || item.buttonLink === "whatsapp"; return `<div class="card service-item service-sub-template-${esc(item.template || "line")}" style="--service-accent:${safeCss(item.color || active.color || "#4966d6")}">${item.imageUrl ? `<img class="service-sub-image" src="${esc(item.imageUrl)}" alt="" />` : ""}<div class="service-item-main"><span class="number">${i + 1}</span><span>${item.iconUrl ? `<img src="${esc(item.iconUrl)}" alt="" />` : esc(item.emoji || "•")}</span><div><b>${esc(title)}</b>${item.description ? `<small>${esc(item.description)}</small>` : ""}</div></div><a class="btn btn-green" href="${esc(href)}" ${external ? 'target="_blank" rel="noopener"' : ""}>◉ ${esc(item.buttonText || "اطلب الخدمة")}</a></div>`; }).join("")}</div><div class="text-center" style="margin-top:2rem"><a class="btn btn-primary" href="${wa(`أريد الاستفسار عن قسم: ${active.title}`)}" target="_blank" rel="noopener">◉ تواصل لجميع خدمات هذا القسم</a></div></div>`;
}
