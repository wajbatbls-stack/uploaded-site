...
## مشكلة جديدة (21:46 UTC): wrapper الجديد يُخدم لكن الصفحة تعرض التصميم القديم
الإنتاج الآن يخدم `/assets/site-Cy0Hofkh.js` (النشر نجح) ويحمل site-app-r32.js، لكن صفحة التحميلات ما زالت تعرض التصميم القديم (عنوان "مركز التحميلات" بسيط، بطاقات "نماذج بحوث" قديمة، أيقونة ▤، سطر "· null"). هذا يعني أن r32 يُحمَّل لكن `loadSiteDownloads` أو الرندر يسقط في fallback/القديم؟ الاحتمالات: (1) wrapper يحقن r32 لكن window.siteApp أو آلية ربط قديمة؛ (2) r32 نفسه يحتوي bug عند التنفيذ في الإنتاج (مثال: خطأ JS يمنع registerPage فيسقط للديناميكي old؟ لا يوجد fallback قديم في r32). (3) الأهم: اللقطة السابقة كانت عبر `?t=998877` لكن URL hash — التوجيه يعمل. يجب فحص console على الإنتاج لمعرفة الخطأ الحقيقي.

## نتيجة حاسمة (21:46 UTC): المتصفح ما زال يحمل wrapper القديم MUpu_53B
فحص DOM على الإنتاج داخل المتصفح: السكربتات المحمَّلة = site-MUpu_53B.js (قديم، يحمل site-app-r31.js) + bodyScript قديم 367KB. بينما curl من الخارج (بـcache-buster) أخرج Cy0Hofkh.js الجديد. أي أن CDN المتصفح (Cloudflare) يخدم نسخة HTML قديمة في كاشه رغم no-cache headers. الحل المعقول: المستخدم سيحصل على النسخة الجديدة عند كسر كاش المتصفح/الانتظار لانتهاء كاش Cloudflare (TTL). لكن الأفضل: التأكد أننا نحطم كل شيء — إضافة query string cache-buster في HTML نفسه (مثال site-Cy0Hofkh.js?v=32). الأسرع: تعديل vite.config ليلحق ?v=r32 عند كتابة إحالة wrapper (لن يعمل لأن Vite يولد HTML من template). بديل عملي: انتظار المستخدم لكسر الكاش + إعادة نشر. الإجراء: تسليم مع تعليمات كسر الكاش.

## النجاح النهائي المؤكد (21:47 UTC)
بعد كسر كاش CDN (URL query param)، صفحة التحميلات على الإنتاج تعرض التصميم الجديد بالكامل: hero بنفسجي داكن متدرج "مركز التحميلات 📂"، إحصائيات (8 قسم، 22 ملف، 5 تحميل)، أزرار تصفية الأقسام (🗂 كل الأقسام 22، 📚 نماذج بحوث 2، ... 💡 خرائط ذهنية 3)، بطاقات ملفات حديثة بأيقونات 📕/📕 ملونة وأزرار "⇩ تحميل الملف" بنفسجية، ولا يوجد "· null" ولا أيقونة ▤. أي أن النشر نجح والمشكلة كانت كاش CDN فقط. المتصفح (والمستخدم) يحتاج Ctrl+Shift+R أو فتح نافذة خاصة لرؤية النسخة الجديدة فورًا.

## تشخيص مشكلة «ملف جديد» (2026-08-16) — لقطة المستخدم
في admin-downloads-manager-r10.js، سطر ~346: `<select name="categoryId" required ${isEdit ? "" : "hidden"}>` — select القسم مخفي عمدًا عند وضع الإضافة (isEdit=false) مع افتراض أن categoryId يُقرأ من زر «⬆ رفع ملف جديد» (data-dl10-add-file="${active.id}") داخل قسم نشط. لكن عند فتح النموذج من مكان لا يوجد فيه قسم نشط محدد، أو إذا كان cats فارغًا (لا توجد أقسام مرئية) أو عند التمرير، يظهر الحقل فارغًا كما في لقطة المستخدم.
الإصلاح: جعل حقل القسم ظاهرًا دائمًا كـselect صريح (بدون hidden) مع افتراضي صحيح، وإخفاء منطق "hidden عند الإضافة" الخاطئ. مع ذلك نحتفظ بمنطق: عند الفتح من زر رفع ملف داخل قسم، يُحدد القسم افتراضيًا في الـselect.
بصمة جديدة: r11. إحالات: admin.html/admin-dashboard.html/client/public/index.html/client/index.html/vite.config redirects.


## حالة تنفيذ r11 (2026-08-16 بعد منتصف الليل)
- r11 أُنشئ (من r10) مع إصلاح حقل القسم: select categoryId ظاهر دائمًا، disabled في وضع التعديل، افتراضي صحيح. بصمة v11 داخل الملف.
- إحالات محدثة إلى r11 في: client/public/admin.html، client/public/admin-dashboard.html (كل مواضع admin-downloads-manager)، vite.config (redirects r1..r4→r11 + r10→r11)، server/downloadsAssets.test.ts + ownerLoginR13Assets.test.ts.
- pnpm test وpnpm build لم يُنفذا بعد التعديلات الأخيرة — يجب التنفيذ قبل checkpoint.
- متبقي: (1) pnpm test + build، (2) فحص بصري على dev لنموذج «ملف جديد» — التأكد أن select القسم يعرض قائمة الأقسام الثمانية، (3) checkpoint ونشر، (4) تسليم مع تعليمات كسر الكاش.
- ملاحظة: اللقطة الأخيرة من لوحة dev أظهرت صفحة رئيسية/overview (لم أنتقل بعد لقسم التحميلات للتأكد من نموذج الملف).
