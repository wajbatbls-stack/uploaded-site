        ...
        <div class="dl10-grid-2">
          ...
          <div class="dl10-field ${!isEdit ? "dl10-field-span" : ""}">
            ...
            <select name="categoryId" required ${isEdit ? "disabled" : ""}>
              ${cats.length === 0 ? "<option value=\"\" selected>لا توجد أقسام — أنشئ قسمًا جديدًا أولًا</option>" : cats.map(c => `<option value="${c.id}" ${Number(file?.categoryId ?? categoryId ?? cats[0].id) === Number(c.id) ? "selected" : ""}>${esc(c.emoji || "📁")} ${esc(c.name)}</option>`).join("")}
            </select>
          </div>
        </div>
        <div class="dl10-field">
