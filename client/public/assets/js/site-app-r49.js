...
// لا تُعرض أي صورة افتراضية: يظهر الشعار فقط إذا أضافه المالك لاحقًا من الإعدادات.
let logoUrl = "";
let siteSettings = {};
let activeDesign = {};
...
function buildTickerContent(text) {
  const source = String(text || "مرحباً بكم في واجبات بلس ⭐ نقدم أفضل الخدمات الأكاديمية ⭐ تواصل معنا على واتساب +966567680470").trim();
  const numbers = [...new Set((source.match(/\+\d[\d\s]{6,}/g) || []).map((number) => number.replace(/\s+/g, "")).filter(Boolean))];
  const message = source.replace(/\+\d[\d\s]{6,}/g, "").replace(/\s{2,}/g, " ").trim();
  return { message: message || "مرحباً بكم في واجبات بلس", numbers };
}
function header() {
  const ticker = buildTickerContent(siteSettings.tickerText);
  const numbers = ticker.numbers.length ? ticker.numbers : ["+" + waNumber()];
  const numberChips = numbers.map((number, index) => {
    const digits = number.replace(/\D/g, "");
    return `<a class="ticker-number-chip" href="https://wa.me/${digits}" target="_blank" rel="noopener" dir="ltr" style="--ticker-delay:${index * .18}s">${esc(number)}</a>`;
  }).join("");
  return `<aside class="ticker ticker-motion" aria-label="معلومات التواصل"><div class="ticker-message-viewport"><p class="ticker-message-track">${esc(ticker.message)}</p></div><div class="ticker-numbers">${numberChips}</div></aside>
...
function sidebar() {
  ...
  return `<div class="sidebar-backdrop ${state.sidebar ? "open" : ""}" data-action="close-sidebar"></div>
    <aside class="sidebar ${state.sidebar ? "open" : ""}">
      <div class="sidebar-head"><a class="brand" href="#/"><span><span class="brand-title">واجبات بلس</span><span class="brand-subtitle">Wajibat Plus</span></span></a><button class="btn-icon" data-action="close-sidebar">×</button></div>
...
async function openReadOnlyModel(fileId, name) {
  downloadReader = { id: Number(fileId), name: String(name || "نموذج للقراءة"), status: "loading", page: 1, pageCount: 0 };
  render();
  try {
    const response = await fetch(`/api/read-only/${encodeURIComponent(String(fileId))}`, { credentials: "same-origin" });
    if (!response.ok) throw new Error("reader unavailable");
    const meta = await response.json();
    if (!downloadReader || Number(downloadReader.id) !== Number(fileId)) return;
    downloadReader = { ...downloadReader, status: "ready", page: 1, pageCount: Number(meta.pageCount || 0) };
  } catch {
    if (!downloadReader || Number(downloadReader.id) !== Number(fileId)) return;
    downloadReader = { ...downloadReader, status: "error" };
  }
  render();
}
...
function dl10Styles() {
  return `<style>
    ...
    .dl10-file-meta{display:flex;align-items:center;gap:.6rem;font-size:.72rem;color:#75829e}
    .dl10-file-desc{display:block}
    .dl10-read-btn{display:inline-flex;align-items:center;justify-content:center;gap:7px;min-height:43px;padding:.65rem 1rem;border:1px solid #c7d0f4;border-radius:12px;background:#f6f7ff;color:#2c3f9b;font-family:inherit;font-size:.85rem;font-weight:900;cursor:pointer;transition:transform .16s cubic-bezier(.23,1,.32,1),background .16s,box-shadow .16s}
    .dl10-read-btn:hover{background:#4f46e5;color:#fff;box-shadow:0 12px 24px rgba(79,70,229,.25);transform:translateY(-2px)}.dl10-read-btn:active{transform:scale(.97)}
    .dl10-read-note{display:flex;align-items:center;justify-content:center;min-height:43px;padding:.65rem .85rem;border:1px dashed #d7ddea;border-radius:12px;background:#fbfcff;color:#78849f;font-size:.75rem;font-weight:800;text-align:center}
    .dl10-reader-overlay{position:fixed;inset:0;z-index:200;display:grid;place-items:center;padding:clamp(.7rem,3vw,2rem);background:rgba(9,17,40,.72);backdrop-filter:blur(8px);animation:dl10-reader-in .18s cubic-bezier(.23,1,.32,1)}
    .dl10-reader-card{display:flex;flex-direction:column;width:min(1080px,100%);height:min(86dvh,860px);overflow:hidden;border:1px solid rgba(255,255,255,.3);border-radius:20px;background:#fff;box-shadow:0 28px 80px rgba(4,10,31,.45)}
    .dl10-reader-head{display:flex;align-items:center;gap:1rem;min-height:64px;padding:.75rem 1rem;background:linear-gradient(110deg,#111c45,#283b88);color:#fff}.dl10-reader-head>div{min-width:0;flex:1}.dl10-reader-head p{margin:0 0 2px;color:#cbd7ff;font-size:.66rem;font-weight:800;letter-spacing:.04em}.dl10-reader-head h2{overflow:hidden;margin:0;color:#fff;font-size:1rem;font-weight:900;text-overflow:ellipsis;white-space:nowrap}.dl10-reader-close{display:inline-flex;align-items:center;justify-content:center;width:38px;height:38px;border:1px solid rgba(255,255,255,.45);border-radius:50%;background:rgba(255,255,255,.08);color:#fff;font-size:1.25rem;cursor:pointer;transition:transform .16s cubic-bezier(.23,1,.32,1),background .16s}.dl10-reader-close:hover{background:rgba(255,255,255,.18);transform:rotate(90deg)}
    .dl10-reader-note{margin:0;padding:.55rem 1rem;background:#fff7df;color:#74561e;font-size:.74rem;font-weight:800;text-align:center}.dl10-reader-frame{position:relative;flex:1;min-height:0;overflow:auto;background:#edf0f7;padding:clamp(.5rem,2vw,1.2rem)}.dl10-reader-frame img{display:block;width:min(100%,920px);height:auto;margin:auto;box-shadow:0 12px 32px rgba(29,42,82,.2);background:#fff}.dl10-reader-loading,.dl10-reader-unavailable{display:grid;place-items:center;min-height:220px;padding:2rem;background:#f7f8fc;color:#5d6c86;text-align:center}.dl10-reader-unavailable b{display:block;color:#24335d;font-size:1.05rem}.dl10-reader-controls{display:flex;align-items:center;justify-content:center;gap:.7rem;padding:.65rem 1rem;border-top:1px solid #e5e9f5;background:#fff}.dl10-reader-controls button{min-width:94px;padding:.52rem .75rem;border:1px solid #c8d0ea;border-radius:10px;background:#f8f9ff;color:#273b89;font:800 .78rem inherit;cursor:pointer}.dl10-reader-controls button:disabled{opacity:.4;cursor:not-allowed}.dl10-reader-controls span{color:#53617f;font-size:.78rem;font-weight:800}
    @keyframes dl10-reader-in{from{opacity:0;transform:scale(.98)}to{opacity:1;transform:scale(1)}}
    ...
    @media (max-width:760px){.dl10-hero{padding:1.8rem 1rem}.dl10-hero h1{font-size:1.5rem}.dl10-grid{grid-template-columns:repeat(auto-fill,minmax(240px,1fr))}.dl10-reader-overlay{padding:0}.dl10-reader-card{width:100%;height:100dvh;border:0;border-radius:0}.dl10-reader-head{min-height:58px}.dl10-reader-note{font-size:.68rem}}
    @media (prefers-reduced-motion:reduce){.dl10-read-btn,.dl10-reader-close{transition:none}.dl10-reader-overlay{animation:none}}
  </style>`;
}
function dl10ReaderKind(file) {
...
function dl10FileCard(file) {
  const canRead = file.readerSupported === true;
  const meta = canRead ? "◉ متاح للقراءة داخل الموقع" : "◌ هذا النوع غير متاح للقراءة داخل الموقع";
  ...
  const action = canRead
    ? `<button type="button" class="dl10-read-btn" data-action="open-read-only" data-reader-id="${Number(file.id)}" data-reader-name="${esc(file.originalName || file.fileName || "نموذج")}">▣ قراءة النموذج</button>`
    : `<span class="dl10-read-note">متاح من الإدارة فقط حتى يُرفع بصيغة قابلة للقراءة.</span>`;
...
function dl10Reader() {
  if (!downloadReader) return "";
  const name = esc(downloadReader.name || "نموذج للقراءة");
  const page = Math.max(1, Number(downloadReader.page || 1));
  const pageCount = Math.max(0, Number(downloadReader.pageCount || 0));
  const view = downloadReader.status === "loading"
    ? `<div class="dl10-reader-loading">جارٍ تجهيز صفحات النموذج للقراءة…</div>`
    : downloadReader.status === "ready" && pageCount > 0
      ? `<img src="/api/read-only/${encodeURIComponent(String(downloadReader.id))}/page/${page}" alt="${name} — الصفحة ${page}" />`
      : `<div class="dl10-reader-unavailable"><div><b>تعذر تجهيز صفحات النموذج للقراءة الآن.</b><p>يرجى المحاولة مرة أخرى لاحقًا.</p></div></div>`;
  const controls = downloadReader.status === "ready" && pageCount > 0 ? `<nav class="dl10-reader-controls" aria-label="تنقل صفحات النموذج"><button type="button" data-action="reader-page" data-reader-page="${page - 1}" ${page <= 1 ? "disabled" : ""}>السابقة</button><span>صفحة ${page} من ${pageCount}</span><button type="button" data-action="reader-page" data-reader-page="${page + 1}" ${page >= pageCount ? "disabled" : ""}>التالية</button></nav>` : "";
  return `<section class="dl10-reader-overlay" data-action="close-read-only" aria-label="قارئ النموذج"><article class="dl10-reader-card" role="dialog" aria-modal="true" aria-labelledby="dl10-reader-title" data-modal-card><header class="dl10-reader-head"><div><p>وضع القراءة فقط</p><h2 id="dl10-reader-title">${name}</h2></div><button type="button" class="dl10-reader-close" data-action="close-read-only" aria-label="إغلاق القارئ">×</button></header><p class="dl10-reader-note">يعرض القارئ صفحات مصوّرة داخل الموقع؛ لا يُرسل ملف PDF إلى جهازك.</p><div class="dl10-reader-frame">${view}</div>${controls}</article></section>`;
}
function downloadsPage() {
  const heading = `<div class="dl10-hero"><h1>📂 مكتبة النماذج</h1><p>نماذج وملفات أكاديمية للقراءة والاطلاع داخل الموقع — واجبات، سيَر ذاتية، عروض تقديمية وأكثر.</p><div class="dl10-stats" id="dl10-hero-stats"></div></div>`;
  if (Array.isArray(siteDownloads?.categories) && siteDownloads.categories.some(cat => Array.isArray(cat.files) && cat.files.length)) {
    ...
    setTimeout(() => {
      ...
      if (statsEl) statsEl.innerHTML = `<div class="dl10-stat"><b>${categories.length.toLocaleString("ar-SA-u-nu-latn")}</b><span>قسم</span></div><div class="dl10-stat"><b>${totalFiles.toLocaleString("ar-SA-u-nu-latn")}</b><span>نموذج للقراءة</span></div><div class="dl10-stat"><b>◉</b><span>دون تنزيل</span></div>`;
    }, 50);
    ...
    return `<div class="container section">${dl10Styles()}${heading}${tabs}${sections}${dl10Reader()}</div>`;
  }
  return `<div class="container section">${dl10Styles()}${heading}<div class="dl10-empty"><span class="glyph">📥</span><b>تجهّز المكتبة حالياً</b><p>لا توجد نماذج قابلة للقراءة بعد. نتابع رفع أحدث النماذج والملفات الأكاديمية لتجدها هنا قريباً.</p></div>${dl10Reader()}</div>`;
}
  ...
  if (channels.length > 0) {
    for (const channel of channels) {
      ...
      if (channel.type === "whatsapp") {
        ...
        value = "+" + number.replace(/^\+/, ""); href = "https://wa.me/" + number.replace(/^\+/, "");
...
function contactPage() {
  ...
  const channelCard = (card, kind, index) => {
    ...
    const inner = href ? `<a class="galleria-channel-value" href="${href}" ${href.startsWith("http") ? 'target="_blank" rel="noopener"' : ""}>${esc(value)}</a>` : `<span class="galleria-channel-value">${esc(value)}</span>`;
    return `<article class="galleria-channel galleria-${kind}"><span class="galleria-channel-num">${String(index + 1).padStart(2, "0")}</span><span class="galleria-channel-icon">${card[0]}</span><div><small>${esc(label)}</small>${inner}</div></article>`;
  };
  ...
  return `<main class="contact-galleria" aria-label="مركز التواصل">
    <header class="galleria-masthead"><div class="galleria-emblem">W+</div><div><p class="galleria-eyebrow">مكتب علاقات واجبات بلس</p><h1>تواصلٌ يليق<br /><strong>بخطوتك القادمة</strong></h1><p class="galleria-lead">قنوات مباشرة وفريق حاضر للاستماع إلى احتياجك وتوجيهك إلى المسار الأنسب دون تعقيد.</p></div><aside class="galleria-response"><span>مستوى الاستجابة</span><b>دعم مباشر<br />بوضوح واهتمام</b><small>اختر إحدى القنوات أو اترك رسالتك في النموذج وسنتابع معك.</small><i></i></aside></header>
    <section class="galleria-channels" aria-label="قنوات التواصل"><div class="galleria-channel-intro"><p>قنواتنا المباشرة</p><h2>اختر القناة الأقرب لك</h2></div><div class="galleria-channel-rail">${allChannels}</div></section>
    <section class="galleria-letter" id="contact-message-form"><aside class="galleria-protocol"><div class="galleria-protocol-head"><p>آلية المتابعة</p><h2>رسالة واحدة<br />تفتح المسار</h2><span>أخبرنا بما تحتاجه، وسيتولى الفريق متابعة التفاصيل معك.</span></div><div class="galleria-protocol-steps"><div class="galleria-protocol-step"><b>01</b><div><strong>اكتب طلبك</strong><small>وضّح الاستفسار أو الخدمة التي تحتاجها.</small></div></div><div class="galleria-protocol-step"><b>02</b><div><strong>نراجع التفاصيل</strong><small>نقرأ الرسالة ونحدد طريقة المتابعة المناسبة.</small></div></div><div class="galleria-protocol-step"><b>03</b><div><strong>نتواصل معك</strong><small>نعود إليك عبر القناة الملائمة بأقرب وقت.</small></div></div></div><a class="galleria-protocol-link" href="${waLink}" target="_blank" rel="noopener"><span>◉</span><div><small>للاستجابة الأسرع</small><b>ابدأ محادثة واتساب</b></div></a></aside><article class="galleria-form"><div class="galleria-form-top"><p>رسالة خاصة إلى الفريق</p><div class="galleria-form-stamp">WP</div></div><h2>كيف نساعدك اليوم؟</h2><p class="galleria-form-intro">املأ البيانات التالية، وسنحرص على فهم طلبك ومتابعته بصورة منظمة.</p><form data-form="contact" class="galleria-contact-form"><div class="galleria-form-line"><div class="field"><label>الاسم الكريم *</label><input name="name" required placeholder="اكتب اسمك هنا" /></div><div class="field"><label>رقم الجوال *</label><input name="phone" required placeholder="05XXXXXXXX" /></div></div><div class="field"><label>البريد الإلكتروني</label><input name="email" type="email" placeholder="example@email.com" dir="ltr" /></div><div class="field"><label>موضوع الرسالة *</label><input name="subject" required placeholder="عن ماذا ترغب بالاستفسار؟" /></div><div class="field"><label>رسالتك *</label><textarea name="message" required placeholder="اكتب تفاصيل طلبك هنا، وسيتابع معك الفريق..."></textarea></div><button class="galleria-submit" type="submit">إرسال الرسالة <span>↗</span></button></form></article></section>
    <section class="galleria-coda"><div><p>تابعنا حيث تفضّل</p><h2>كل وسائل التواصل في مكان واحد</h2></div><div class="galleria-socials">${socialIcons || `<span class="text-muted">لا توجد روابط تواصل اجتماعي بعد</span>`}</div><a href="javascript:void(0)" data-action="contact-scroll">العودة إلى نموذج الرسالة ↑</a></section>
  </main>`;
}
...
document.addEventListener("click", (event) => {
  ...
  if (action === "open-read-only") { const fileId = Number(target.dataset.readerId); if (Number.isInteger(fileId) && fileId > 0) { recordContentView(`/downloads/read/${fileId}`); void openReadOnlyModel(fileId, target.dataset.readerName || "نموذج للقراءة"); } }
  if (action === "reader-page") { const page = Number(target.dataset.readerPage); if (downloadReader && Number.isInteger(page) && page >= 1 && page <= Number(downloadReader.pageCount || 0)) { downloadReader = { ...downloadReader, page }; render(); } }
  if (action === "close-read-only") { if (target.classList.contains("dl10-reader-overlay") && event.target !== target) return; downloadReader = null; render(); }
