...
function dl10Styles() {
  return `<style>
    ...
    .dl10-reader-note{margin:0;padding:.55rem 1rem;background:#fff7df;color:#74561e;font-size:.74rem;font-weight:800;text-align:center}.dl10-reader-frame{flex:1;min-height:0;background:#edf0f7}.dl10-reader-frame iframe{display:block;width:100%;height:100%;border:0;background:#fff}.dl10-reader-frame img{display:block;max-width:100%;max-height:100%;margin:auto;object-fit:contain;background:#edf0f7}.dl10-reader-unavailable{display:grid;place-items:center;min-height:220px;padding:2rem;background:#f7f8fc;color:#5d6c86;text-align:center}.dl10-reader-unavailable b{display:block;color:#24335d;font-size:1.05rem}
...
function dl10PdfViewerUrl(url) {
  try {
    const absolute = new URL(String(url || ""), window.location.origin).toString();
...
function dl10FileCard(file) {
  ...
  const kind = dl10ReaderKind(file);
  const canRead = Boolean(url) && kind !== "unsupported";
  ...
  const action = canRead
    ? `<button type="button" class="dl10-read-btn" data-action="open-read-only" data-reader-url="${esc(url)}" data-reader-name="${esc(file.originalName || file.fileName || "نموذج")}" data-reader-kind="${kind}">▣ قراءة النموذج</button>`
...
function dl10Reader() {
  ...
  const url = String(downloadReader.url || "").replace(/["\\]/g, "");
  const kind = downloadReader.kind || "unsupported";
  const pdfViewerUrl = kind === "pdf" ? dl10PdfViewerUrl(url) : "";
  const view = kind === "image"
    ? `<img src="${url}" alt="${name}" />`
    : kind === "pdf" && pdfViewerUrl
      ? `<iframe src="${esc(pdfViewerUrl)}" title="${name}" referrerpolicy="no-referrer" allow="fullscreen"></iframe>`
      : kind === "text"
        ? `<iframe src="${url}" title="${name}" referrerpolicy="no-referrer"></iframe>`
      : `<div class="dl10-reader-unavailable"><div><b>لا تتوفر معاينة لهذا النوع من الملفات.</b><p>ارفع النموذج بصيغة PDF أو صورة أو ملف نصي ليظهر للقارئ داخل الموقع.</p></div></div>`;
  return `<section class="dl10-reader-overlay" data-action="close-read-only" aria-label="قارئ النموذج"><article class="dl10-reader-card" role="dialog" aria-modal="true" aria-labelledby="dl10-reader-title" data-modal-card><header class="dl10-reader-head"><div><p>وضع القراءة فقط</p><h2 id="dl10-reader-title">${name}</h2></div><button type="button" class="dl10-reader-close" data-action="close-read-only" aria-label="إغلاق القارئ">×</button></header><p class="dl10-reader-note">هذا النموذج مخصص للقراءة داخل الموقع فقط، ولا يظهر له زر تنزيل.</p><div class="dl10-reader-frame">${view}</div></article></section>`;
}
...
document.addEventListener("click", (event) => {
  ...
  if (action === "open-read-only") { downloadReader = { url: target.dataset.readerUrl || "", name: target.dataset.readerName || "نموذج للقراءة", kind: target.dataset.readerKind || "unsupported" }; recordContentView(`/downloads/read/${encodeURIComponent(downloadReader.name)}`); render(); }
