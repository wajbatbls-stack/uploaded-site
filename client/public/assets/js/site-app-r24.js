...
function partnersPage() {
  ...
  const partnerCard = (item) => {
    const hasLink = Boolean(item.link) && String(item.link).trim().length > 3;
    const href = hasLink ? esc(String(item.link).trim()) : wa("أريد الاستفسار عن " + item.name);
    ...
    return `<a class="partner-pro-card" href="${href}" target="_blank" rel="noopener" style="${styles}">
      ...
      ${hasLink ? `<span class="partner-pro-link">زيارة الموقع &#8599;</span>` : ""}
    </a>`;
  };
  ...
  return `<div class="container section">
    ...
    <div class="partners-stats-strip">
      ...
      <div class="partners-stat"><div class="partners-stat-num">${new Set((dynamicPartners || []).map(p => p.city).filter(Boolean)).size}</div><div class="partners-stat-label">مدينة ومنطقة مختلفة</div></div>
