...
const waNumber = (custom = null) => {
  ...
  const n966 = digits.match(/966\d{9,}/)?.[0];
  if (n966) return n966;
  const n5 = digits.match(/^5\d{8,9}$/)?.[0];
  if (n5) return "966" + n5;
  return digits || "966567680470";
};
...
function partnersPage() {
  ...
  const partnerCard = (item) => {
    ...
    const hasWhatsApp = !hasWebsite && isWhatsAppNumber(rawLink);
    const href = hasWebsite ? esc(rawLink) : wa("أريد الاستفسار عن " + item.name);
    ...
    const cleanWa = hasWhatsApp ? waNumber(rawLink) : null;
    const waHref = waNumber ? wa("أريد الاستفسار عن " + item.name) : wa("أريد الاستفسار عن خدماتكم");
    const buttons = [
      hasWebsite ? `<a class="partner-pro-btn partner-pro-btn-site" href="${esc(rawLink)}" target="_blank" rel="noopener" onclick="event.stopPropagation()">زيارة الموقع &#8599;</a>` : "",
      hasWhatsApp ? `<a class="partner-pro-btn partner-pro-btn-wa" href="https://wa.me/${esc(cleanWa)}?text=${encodeURIComponent("أريد الاستفسار عن " + item.name)}" target="_blank" rel="noopener" onclick="event.stopPropagation()">واتساب &#9993;</a>` : "",
    ].filter(Boolean).join("");
    return `<a class="partner-pro-card" href="${href}" target="_blank" rel="noopener" style="${styles}">
      ...
      ${item.description ? `<span class="partner-pro-desc">${esc(item.description)}</span>` : ""}
      ${buttons ? `<span class="partner-pro-actions">${buttons}</span>` : ""}
    </a>`;
  };
