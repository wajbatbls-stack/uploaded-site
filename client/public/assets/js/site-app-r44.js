...
function contactPage() {
  ...
  const channelCard = (card, kind, index) => {
    ...
    const inner = href ? `<a class="atelier-route-value" href="${href}" ${href.startsWith("http") ? 'target="_blank" rel="noopener"' : ""}>${esc(value)}</a>` : `<span class="atelier-route-value">${esc(value)}</span>`;
    return `<article class="atelier-route atelier-${kind}"><span class="atelier-route-num">${String(index + 1).padStart(2, "0")}</span><span class="atelier-route-icon">${card[0]}</span><div><small>${esc(label)}</small>${inner}</div></article>`;
  };
  ...
  return `<main class="contact-atelier" aria-label="مركز التواصل">
    <section class="atelier-welcome"><div class="atelier-mark"><span>W+</span></div><div class="atelier-welcome-copy"><p>مركز علاقات واجبات بلس</p><h1>نستمع بتفاصيل<br /><strong>تليق بطموحك</strong></h1></div><div class="atelier-welcome-note"><span>وقت الاستجابة</span><b>دعم مباشر</b><small>من القنوات المتاحة أدناه</small></div></section>
    <section class="atelier-workspace" id="contact-message-form"><aside class="atelier-directory"><div class="atelier-directory-head"><span>دليل التواصل</span><h2>اختر مسارك</h2><p>كل قناة تضاف من لوحة الإدارة تظهر هنا مباشرة وبشكل مستقل.</p></div><div class="atelier-route-list">${allChannels}</div><a class="atelier-whatsapp" href="${waLink}" target="_blank" rel="noopener"><span>◉</span><div><small>استجابة سريعة</small><b>تواصل عبر واتساب</b></div><i>←</i></a></aside><article class="atelier-message"><div class="atelier-message-top"><span>رسالة خاصة</span><div class="atelier-stamp">WP</div></div><div class="atelier-title"><p>نموذج المراسلة</p><h2>كيف يمكننا مساعدتك؟</h2><span></span></div><form data-form="contact" class="atelier-contact-form"><div class="atelier-form-line"><div class="field"><label>الاسم الكريم *</label><input name="name" required placeholder="اكتب اسمك هنا" /></div><div class="field"><label>رقم الجوال *</label><input name="phone" required placeholder="05XXXXXXXX" /></div></div><div class="field"><label>البريد الإلكتروني</label><input name="email" type="email" placeholder="example@email.com" dir="ltr" /></div><div class="field"><label>موضوع الرسالة *</label><input name="subject" required placeholder="عن ماذا ترغب بالاستفسار؟" /></div><div class="field"><label>رسالتك *</label><textarea name="message" required placeholder="اكتب تفاصيل طلبك هنا، وسيتابع معك الفريق..."></textarea></div><button class="atelier-submit" type="submit">إرسال الرسالة <span>↗</span></button></form></article></section>
    <section class="atelier-footer"><div><p>ابقَ على مقربة</p><h2>كل وسائلنا بين يديك</h2></div><div class="atelier-socials">${socialIcons || `<span class="text-muted">لا توجد روابط تواصل اجتماعي بعد</span>`}</div><a href="javascript:void(0)" data-action="contact-scroll">العودة إلى نموذج الرسالة ↑</a></section>
  </main>`;
}
