...
(function () {
  ...
  const manager = {
    ...
    async uploadPartnerLogo(file) {
      ...
      if (!form) return;
      if (!form.id) {
        form.logoPreviewUrl = await this.fileToDataUrl(file);
        this.state.pendingLogoFile = file;
        this.state.logoUploadState = "pending";
        this.render();
        this.toast("تم اختيار الشعار — سيُرفع تلقائيًا عند حفظ الجهة الجديدة", "ok");
        return;
      }
    ...
    /* --------------------- العمليات --------------------- */
    emptyForm() {
      return { id: null, name: "", city: "", description: "", kind: "جامعة", link: "", logoUrl: null, logoPreviewUrl: null, shape: "card", accentColor: "#4966d6", textColor: "#ffffff", backgroundColor: "#eef1f8", borderColor: null };
    },
    startNew() {
      this.state.editing = this.emptyForm();
      this.state.logoUploadState = null;
      this.state.pendingLogoFile = null;
      this.render();
    },
    cancel() {
      this.state.editing = null;
    ...
    async save() {
      ...
      try {
        ...
        if (this.state.pendingLogoFile && form.id) {
          const pendingFile = this.state.pendingLogoFile;
          this.state.logoUploadState = "uploading";
          this.render();
          const result = await this.uploadLogo(pendingFile, form.id);
          form.logoUrl = result?.url || null;
          form.logoPreviewUrl = null;
          this.state.pendingLogoFile = null;
          this.state.logoUploadState = "done";
          logoWasUploaded = true;
        }
        this.toast(logoWasUploaded ? "تم حفظ الجهة ورفع شعارها بنجاح — يظهران فورًا للزوار" : "تم حفظ الشريك في قاعدة البيانات — يظهر فورًا في صفحة الزائر", "ok");
    ...
    pickLogoFile() {
      const input = this.container?.querySelector("[data-partner-logo-input]");
    ...
    render() {
      ...
      const completePartners = state.partners.filter(p => p.logoUrl && p.link && p.description).length;
      const head = `
        <div class="pm-head">
          <div class="pm-head-right">
            <span class="pm-head-badge">مركز إدارة الشراكات</span>
            <h2 class="pm-head-title">🏛️ شركاء النجاح</h2>
            <p class="pm-head-sub">أنشئ صفحة تعريف متكاملة لكل جامعة: شعار موثوق، نبذة للزوار، ورابط مباشر للموقع الرسمي.</p>
            ${state.loaded ? `<div class="pm-head-stats"><span><b>${totalPartners}</b> جهة</span><span><b>${completePartners}</b> ملفات تعريف مكتملة</span><span><b>${state.partners.filter(p => p.isVisible !== false).length}</b> ظاهرة للزوار</span></div>` : ""}
          </div>
      ...
      root.querySelectorAll("[data-partner-logo-input]").forEach(input => input.addEventListener("change", () => {
        const file = input.files?.[0];
        if (file) void this.uploadPartnerLogo(file);
      }));
    },
    ...
    partnerRow(partner) {
      ...
      return `<article class="pm-card" style="--pm-accent:${partner.accentColor || "#4966d6"};--pm-bg:${partner.backgroundColor || "#eef1f8"}">
        ...
        <div class="pm-card-body">
          ...
          ${partner.description ? `<p class="pm-card-description">${this.esc(partner.description)}</p>` : `<p class="pm-card-description pm-missing">أضف وصفًا مختصرًا للتعريف بالجهة</p>`}
          <div class="pm-card-signals">
            <span class="${partner.logoUrl ? "ready" : "missing"}">${partner.logoUrl ? "◉ شعار مرفوع" : "○ بدون شعار"}</span>
            <span class="${partner.link ? "ready" : "missing"}">${partner.link ? "↗ موقع رسمي" : "○ بدون رابط"}</span>
          </div>
    ...
    formSection(form) {
      const previewUrl = form.logoPreviewUrl || form.logoUrl;
      const logoPreview = previewUrl
        ? `<img src="${this.esc(previewUrl)}" alt="شعار الجهة" class="pm-logo-preview-img" />`
      ...
      const uploadBtnText = this.state.logoUploadState === "uploading" ? "⏳ جارٍ رفع الشعار..."
        : this.state.logoUploadState === "done" ? "✔ تم رفع الشعار — تغيير الشعار"
        : this.state.logoUploadState === "pending" ? "✔ تم اختيار الشعار — يمكنك تغييره"
        : (previewUrl ? "تغيير الشعار من الجهاز" : "📤 رفع الشعار من الجهاز");
      const doneBanner = this.state.logoUploadState === "done"
        ? `<div class="pm-upload-ok">✔ تم رفع الشعار بنجاح ويظهر للزوار الآن</div>`
        : this.state.logoUploadState === "pending" ? `<div class="pm-upload-pending">✔ تم اختيار الشعار. اضغط «حفظ الجهة» لرفعه تلقائيًا.</div>` : "";
      return `<div class="pm-editor">
        ...
        <div class="pm-editor-grid">
          <div class="pm-edit-side">
            ...
            <input type="file" accept="image/png,image/jpeg,image/webp,image/gif,image/svg+xml" class="pm-logo-file-input" data-partner-logo-input aria-label="اختيار شعار الجهة" />
            ${doneBanner}
            <small class="pm-note">يمكنك اختيار الشعار الآن؛ للجهة الجديدة يُرفع تلقائيًا عند الحفظ، وللجهة المحفوظة يتحدث فورًا.</small>
          </div>
          <div class="pm-edit-fields">
            ...
            <label><span>رابط الموقع الرسمي — يفتح للزائر عند الضغط على الجهة</span><input type="url" dir="ltr" maxlength="512" value="${this.esc(form.link || "")}" placeholder="https://university.edu.sa" data-partner-prop="link" /></label>
            <label><span>وصف الجهة للزوار</span><textarea maxlength="1200" rows="3" placeholder="اكتب نبذة واضحة عن الجامعة أو جهة الشراكة" data-partner-prop="description">${this.esc(form.description || "")}</textarea></label>
            <div class="pm-looks">
              ...
              <label><span>معاينة الشكل</span>
                <div class="pm-preview-box" data-preview-shape="${form.shape || "card"}" style="background:${form.backgroundColor || "#eef1f8"};border-color:${form.borderColor || form.accentColor || "#4966d6"};color:${form.accentColor || "#4966d6"}">
                  ${previewUrl ? `<img src="${this.esc(previewUrl)}" alt="" />` : "🏛"}
                </div></label>
            </div>
            <div class="pm-colors">
  ...
  (function injectPartnerManagerCss() {
    if (document.getElementById("partner-manager-style-r18")) return;
    const style = document.createElement("style");
    style.id = "partner-manager-style-r18";
...
.pm-edit-side{display:flex;flex-direction:column;gap:.55rem;align-items:stretch;position:relative}
...
.pm-logo-file-input{position:absolute;width:1px;height:1px;opacity:0;overflow:hidden;pointer-events:none}
...
/* هوية لوحة شركاء النجاح r17 */
.pm-head{position:relative;isolation:isolate;overflow:hidden;padding:1.2rem 1.25rem;border-radius:22px;background:radial-gradient(circle at 8% 8%,rgba(102,224,255,.22),transparent 25%),radial-gradient(circle at 92% 100%,rgba(140,107,255,.33),transparent 35%),linear-gradient(135deg,#10284d 0%,#183d77 51%,#392776 100%);box-shadow:0 18px 42px rgba(18,42,84,.24)}
.pm-head:before{content:"";position:absolute;inset:0;z-index:-1;opacity:.25;background-image:linear-gradient(90deg,rgba(255,255,255,.07) 1px,transparent 1px),linear-gradient(rgba(255,255,255,.07) 1px,transparent 1px);background-size:26px 26px}
.pm-head-badge{color:#eef7ff;background:rgba(255,255,255,.14);border-color:rgba(255,255,255,.25);padding:.27rem .66rem}.pm-head-title{color:#fff;font-size:1.48rem;font-weight:900;letter-spacing:-.02em}.pm-head-sub{color:#d9e7ff;line-height:1.7}.pm-head-stats{display:flex;flex-wrap:wrap;gap:.42rem;margin-top:.75rem}.pm-head-stats span{font-size:.68rem;color:#e9f3ff;background:rgba(5,15,40,.22);border:1px solid rgba(255,255,255,.16);border-radius:999px;padding:.3rem .55rem}.pm-head-stats b{color:#fff;font-size:.8rem}
.pm-btn-add-new{position:relative;z-index:1;padding:.78rem 1.25rem;background:linear-gradient(135deg,#63d8ff 0%,#8d7dff 100%);color:#102448;border:1px solid rgba(255,255,255,.6);box-shadow:0 10px 22px rgba(0,0,0,.23);font-weight:900}.pm-btn-add-new:hover{box-shadow:0 14px 28px rgba(0,0,0,.3)}
.pm-grid{grid-template-columns:repeat(auto-fill,minmax(220px,1fr));gap:1rem}.pm-card{border:1px solid #e2e8f3;border-radius:19px;background:linear-gradient(145deg,#fff,#fbfcff);box-shadow:0 8px 22px rgba(31,53,90,.08)}.pm-card:hover{border-color:var(--pm-accent);box-shadow:0 17px 34px rgba(28,54,99,.16)}.pm-card-top{height:108px;background:radial-gradient(circle at 20% 20%,rgba(255,255,255,.94),transparent 37%),linear-gradient(135deg,var(--pm-bg) 0%,#fff 130%);border-bottom:1px solid rgba(92,116,163,.1)}.pm-logo{width:76px;height:76px;border-radius:13px;background:#fff;padding:.25rem;box-shadow:0 7px 18px rgba(23,47,91,.12)}.pm-card-body{padding:.72rem .8rem .8rem}.pm-name{font-size:.9rem;font-weight:900;color:#16253f}.pm-meta{color:#738198;font-weight:700}.pm-card-description{margin:.15rem 0 .2rem;font-size:.7rem;line-height:1.55;color:#52637c;min-height:2.25em}.pm-card-description.pm-missing{color:#a28b6b;font-style:italic}.pm-card-signals{display:flex;flex-wrap:wrap;gap:.3rem;margin:.05rem 0 .25rem}.pm-card-signals span{font-size:.62rem;font-weight:800;border-radius:999px;padding:.22rem .42rem}.pm-card-signals .ready{color:#0f7447;background:#e4f8ed}.pm-card-signals .missing{color:#a36517;background:#fff5e6}
.pm-editor{border:1px solid #dbe5f4;border-radius:20px;background:linear-gradient(145deg,#fff,#f9fbff);box-shadow:0 20px 48px rgba(32,69,138,.14)}.pm-editor-head{margin:-1rem -1rem .95rem;padding:.95rem 1rem .8rem;background:linear-gradient(90deg,#f0f5ff,#fbfaff);border-bottom:1px solid #dce5f2;border-radius:20px 20px 0 0}.pm-logo-upload-btn{padding:.66rem;border-radius:11px;background:linear-gradient(135deg,#edf5ff,#f4efff);color:#2c54aa;font-weight:900}.pm-upload-pending{font-size:.72rem;font-weight:800;color:#225b9f;background:#eaf3ff;border:1px solid #bdd8f7;border-radius:10px;padding:.48rem .52rem;width:100%;text-align:center;animation:pm-pop .28s ease-out}
@media (max-width:768px){.pm-head{padding:1rem;border-radius:18px}.pm-head-title{font-size:1.28rem}.pm-head-sub{font-size:.76rem}.pm-btn-add-new{width:100%;min-height:46px}.pm-edit-logo-preview{min-height:150px}.pm-grid-head{font-size:.74rem}}
@media (max-width:560px){.pm-section{margin-inline:-.1rem}.pm-grid{grid-template-columns:1fr;gap:.75rem}.pm-card-top{height:100px}.pm-card{border-radius:16px}.pm-inline,.pm-looks{grid-template-columns:1fr}.pm-colors{grid-template-columns:1fr 1fr}.pm-editor{padding:.8rem;border-radius:16px}.pm-editor-head{margin:-.8rem -.8rem .8rem;padding:.78rem .8rem}.pm-step{font-size:.64rem}.pm-step:nth-of-type(2){display:none}.pm-editor-foot{position:sticky;bottom:0;background:#fff;padding:.7rem 0 .2rem;z-index:2}.pm-btn-save-big,.pm-btn-flat{min-height:46px;flex:1;padding:.6rem}.pm-card-actions .pm-btn{min-height:35px}.pm-head-stats{gap:.28rem}.pm-head-stats span{font-size:.61rem}}
`;
