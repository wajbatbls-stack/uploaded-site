## sv24 — 2026-08-16: تشخيص وإصلاح تداخل النوافذ

## sv25 — 2026-08-16: فحص محلي لواجهة شركاء النجاح r17

- في `http://localhost:3000/admin-sv24.html` ظهرت واجهة r17 الجديدة و33 جهة شراكة.
- نموذج الإضافة يعرض فعليًا: اسم الجهة، النوع، المدينة، رابط الموقع الرسمي، وصف الجهة، قالب البطاقة وألوانها، مع زر رفع الشعار.
- فحص DOM أكد وجود حقل ملف فعّال باسم `data-partner-logo-input` يقبل PNG/JPEG/WebP/GIF/SVG. الحقل مضبوط بعرض 1px خلف زر الرفع لأسباب تصميمية، وهذا يمنع أداة الاختبار الآلية من إيجاده مباشرةً؛ التفاعل اليدوي عبر زر الرفع يبقى المسار المقصود للمستخدم.
- بطاقة جامعة طيبة تظهر شعارًا محفوظًا ووصفًا ورابطًا رسميًا، بينما البطاقات غير المكتملة تعرض بوضوح حالة «بدون شعار» و«بدون رابط» بدلاً من إخفاء الحقول.
- اختبار حفظ فعلي: أُنشئت جهة مؤقتة باسم `[اختبار مؤقت] شريك sv25` مع رابط `https://example.edu` ووصف تجريبي وشعار SVG. بعد الحفظ تغيّر العدد من 33 إلى 34، وظهر رابط الشعار المحفوظ `/manus-storage/wajbat-plus/partners/30001_e33cc0d5.svg` في محرر الجهة ومعاينتها، بما يثبت أن الاتصال بالقاعدة ومسار رفع الشعار يعملان. يجب حذف الجهة الاختبارية بعد فحص واجهة الزوار.

### متابعة sv25 بعد checkpoint 5dba869d
- أُزيل شريك الاختبار بعد التحقق، وعاد العدد إلى **33** دون تعديل للجهات الأصلية. نجحت اختبارات المشروع (**98/98**) والبناء المحلي، مع تطابق MD5 لحزمة r17 بين المصدر و`dist`: `2792a7997bec99b2633d409af37a3e30`.
- لكن الفحص الحي على `https://uploadplus-47dkogbk.manus.space/admin-sv24.html?release=sv25-5dba869d&probe=1` كشف أن الخادم يعيد نقطة دخول بتاريخ `Sun, 16 Aug 2026 18:39:44 GMT` تحتوي فقط `admin-partners-manager-r16.js?v=partners-r16`، بينما مستودع الإصدار `5dba869` نظيف ويحوي `admin-sv24.html` بإحالة r17. لا يُسلَّم sv25 قبل إنشاء نقطة دخول ذات اسم إصدار جديد والتحقق منها حيًا.
- عقب نشر checkpoint `8592f1b0`، فتح المسار الجديد `https://uploadplus-47dkogbk.manus.space/admin-sv25.html?release=sv25-8592f1b0&probe=live-r17` أعاد تطبيق الزوار بعنوان «واجبات بلس | خدمات أكاديمية سعودية» بدل واجهة الإدارة؛ أي إن خادم الإنتاج لم يقدّم بعد ملف الإدخال الثابت الجديد كما يجب. يستلزم ذلك فحص استجابة HTTP ومسار إنتاج الملفات قبل التسليم.
- تشخيص HTTP الحي عند 00:11 GMT: المساران القياسيان `/admin.html` و`/admin-sv24.html` يعيدان `200` بآخر تعديل `Mon, 17 Aug 2026 00:06:40 GMT` وبإحالات `admin-partners-manager-r17`؛ أما اسم `/admin-sv25.html` الجديد فيقع على مسار SPA الاحتياطي رغم وجوده في `dist` محليًا. لذلك سيكون رابط التسليم الرسمي `/admin-sv24.html` لأنه المسار الحي الذي يعرض فعلًا r17. فحص متصفح حي لهذا الرابط أكمل المصادقة وفتح لوحة المالك بعد رسالة التحقق، مع ظهور «شركاء النجاح 33» في القائمة.
- تحقق حي نهائي من `admin-sv24.html` بعد النشر: فُتح قسم «شركاء النجاح» ويعرض 33 جهة مع أزرار الإضافة والتعديل والإظهار/الإخفاء والحذف. كشف المتصفح أكد أن `window.WajbatPartnersManager` مفعّل وأن الحزمة المحمّلة هي بالضبط `admin-partners-manager-r17.js?v=partners-r17` من الدومين المنشور.

### تصحيح r18 قيد النشر
- السبب البرمجي الإضافي: كان `renderPartners()` في `admin-app-r30.js` يعيد إنشاء المحرر التقليدي بعد تحميل الحزمة الجديدة. أصبح الآن يفوض العرض إلى `WajbatPartnersManager.render()` عند توافره، ثم أُصدرت الحزمة باسم جديد `admin-partners-manager-r18.js` لتجاوز أي نسخة مخزنة.
- تحقق محليًا على `admin-sv24.html?release=sv25-r18-local`: الحزمة الفعالة هي `admin-partners-manager-r18.js`، و`manager=true` و`hasNewLayout=true` و`addButton=true` و`editorFieldContract=true`. ظهرت واجهة الشركاء الجديدة مع **33** جهة، وحالة شعار جامعة طيبة ووصفها ورابطها الرسمي.
- فحوصات الإصدار: اجتاز المشروع **98/98** اختبارًا واكتمل `pnpm build` بنجاح قبل نشر r18.

### تحقق الإنتاج بعد الإصدار 01a91153
- رابط الإدارة الجديد الحي: `https://uploadplus-47dkogbk.manus.space/admin-sv24.html?release=01a91153`.
- ظهر عطل HTTPS مؤقت أثناء انتشار الإصدار في أول محاولة، ثم فُتح الرابط الجديد بنجاح في المحاولة التالية وظهرت شاشة دخول الإدارة بعنوان «لوحة المالك — إصدار sv24».
- نقطة الدخول الجديدة مستقلة عن `/admin` المؤقت وتستخدم ترويسات عدم التخزين وتحمل إحالة مدير التحميلات r20.

تم تحديد السبب الجذري لسلوك نافذتي «إضافة نموذج» و«تعديل نموذج»: كانت طبقة الخلفية `dfa17-backdrop` تحمل `z-index:85`، بينما كانت بطاقة النافذة `dfa17-card` بلا قيمة `z-index`، فتقع الخلفية عمليًا فوق البطاقة وتلتقط النقرات، بما في ذلك نقرات الحقول والأزرار. لذلك كانت النافذة تبدو ضبابية وتغلق أو لا تستجيب عند النقر. أُنشئت حزمة `admin-downloads-manager-r20.js`، وفيها الخلفية `z-index:85` والبطاقة `z-index:86` مع تثبيت `position` و`pointer-events`، ثم حُدّثت إحالات صفحة الإدارة والبناء والاختبارات إلى r20.

تم تشغيل الاختبارات: **25 ملف اختبار / 98 اختبارًا ناجحًا**، واكتمل بناء الإنتاج. كما تطابق MD5 بين مصدر r20 ونسخة الإنتاج المحلية: `deb0740196b203aaf81431417e7ce209`. تبقى خطوة الفحص التفاعلي النهائي على النافذة المفتوحة ثم النشر والتحقق الحي.

فحص تفاعلي في المعاينة بعد تسجيل دخول صحيح: فُتحت نافذة «إضافة نموذج جديد»، واستقبل حقل الاسم وقائمة القسم النقر والكتابة دون انغلاق النافذة. فحص DOM الفعلي أكد أن البطاقة ظاهرة وقابلة للنقر، والحقل غير معطل وغير للقراءة فقط، و`elementFromPoint` فوق وسط الحقل يعيد عنصر `INPUT` نفسه؛ الخلفية والبطاقة تعملان بقيم طبقات فعلية صريحة (`0` و`1`) داخل سياق النافذة، ومن دون `filter` أو `backdrop-filter`.

فُتحت نافذة «تعديل النموذج» لملف أصلي، وتأكد الفحص من أن بطاقة التعديل فوق الخلفية وأن حقل الاسم قابل للكتابة. أُدخل نص مؤقت في حقل الوصف ثم اختُبر زر «إلغاء» بنجاح؛ أُغلقت النافذة ولم يُحفَظ أي تغيير، فبقيت البيانات الأصلية محفوظة. منتقي الملف موجود، متصل بالنافذة، غير معطل، ويقبل اختيار الاستبدال؛ لا حاجة لإنشاء ملف تجريبي أو تعديل ملفات المستخدم للتحقق من التفاعل.
...
## تحقق حي نهائي — sv23 منشور (14:31 UTC)
النشر نجح: إشعار "Deployment successful" على uploadplus-47dkogbk.manus.space. client/public/index.html الحي الآن يحمل سكربت __wpV20Purge كاملًا (مفتاح __wp_purge_v20 + تسجيل /sw-purgex-v20.js?u=v20) و sw-purgex-v20.js حي بـ 200. ملف admin-downloads-manager-r19.js حي بـ 200 و md5 مصدر = dist (c5a7543c15a614d9f1aa0b6e5f09c2cf). ملاحظة: curl مبكر لـ admin.html أثناء فترة النشر أرى r18/v19 (نسخة ما قبل الـ checkpoint) — يجب إعادة التحقق من admin.html بعد اكتمال النشر (قد يستغرق لحظات). الـ index الحي المحدّث يؤكد أن النشر الجديد وصل.
المصدر الحي للإحالات في admin.html المطلوب بعد النشر: admin-downloads-manager-r19 (3 مرات: script src + chain + preload) + sw-purgex-v20 + __wp_purge_v20.
الخطوة المتبقية: إعادة curl لـ admin.html وindex.html حيًا للتوثيق ثم رسالة التسليم النهائية.

## تحقق حي نهائي مؤكد — 14:33 UTC
admin.html المنشور حيًا الآن يحمل: admin-downloads-manager-r19 (3 مرات)، sw-purgex-v20.js، __wp_purge_v20 — أي أن sv23 منشور بالكامل على الإنتاج. md5 حي = 249f85866924efdb25e5a50bcde3ce2b. كل بنود sv23 مكتملة، بقي تسليم المستخدم.

## sv25 — تحقق محلي من إدارة شركاء النجاح r17 (2026-08-16)
- نقطة الدخول المحلية `http://localhost:3000/admin-sv24.html` حمّلت مدير الشركاء r17 بنجاح.
- فتح محرر «جامعة طيبة» أظهر حقولًا فعلية ومتصلة: زر **تغيير الشعار من الجهاز**، الاسم، النوع، المدينة، رابط الموقع الرسمي (`https://www.taibahu.edu.sa/`)، وصف الجهة، قالب بطاقة الزائر، وألوان الشعار والنص والخلفية والإطار.
- معاينة الشعار ظاهرة داخل المحرر، مع رسالة تشرح أن اختيار الشعار قبل الحفظ للجهة الجديدة يرفع تلقائيًا عند الحفظ، وللجهة المحفوظة يتحدث فورًا.
- بطاقات القائمة الجديدة تعرض حالات «شعار مرفوع/بدون شعار» و«موقع رسمي/بدون رابط» ووصفًا أو تنبيهًا واضحًا. أُجري اختبار كتابة للوصف ثم إلغاء دون حفظ، ولم تتغير بيانات الشركاء الحالية.
- فحص المتصفح عند 18:58: الحقول ظاهرة وقابلة للتفاعل داخل محرر جامعة طيبة (`input type=url` للرابط و`textarea` للوصف وbutton لتغيير الشعار). لقطة الفحص: `/home/ubuntu/screenshots/localhost_2026-08-16_18-58-36_9366.webp`.
- فحص مرئي محلي عند 23:54: التصميم الجديد r17 يظهر رأسًا بثلاث خطوات (الشعار/البيانات/مظهر البطاقة)، حقل رابط الموقع الرسمي، حقل الوصف، معاينة شعار، وخيارات قالب وألوان؛ والقائمة تعرض بطاقات مصغرة مع مؤشرات «شعار مرفوع/بدون شعار» و«موقع رسمي/بدون رابط». مصدر الفحص: `http://localhost:3000/admin-sv24.html`، لقطة: `/home/ubuntu/screenshots/localhost_2026-08-16_23-54-50_4348.webp`.
- اختبار وظيفي مكتمل: أُنشئت جهة مؤقتة باسم «[اختبار مؤقت] شريك sv25» مع رابط رسمي ووصف وشعار SVG؛ ظهرت في الإدارة بحالتي «شعار مرفوع» و«موقع رسمي»، ثم ظهرت بياناتها وشعارها في صفحة شركاء النجاح للزوار. بعد ذلك حُذفت الجهة الاختبارية فقط من الإدارة برسالة «تم حذف الشريك نهائيًا»، وتحقق DOM من عدم وجود الاسم وعودة بطاقات الشركاء إلى **33** دون تغيير الجهات الأصلية.

## sv25 — تحقق r31 محلي قبل النشر (2026-08-17)
- فتح «شركاء النجاح» في `admin-sv24.html?release=sv25-r31-local` يعرض مدير r18 المتخصص بدل القائمة التقليدية.
- فتح «تعديل» لجامعة طيبة يعرض محررًا واحدًا بثلاث خطوات، مع حقول **تغيير الشعار من الجهاز** و**رابط الموقع الرسمي** و**وصف الجهة للزوار** وزر الحفظ، وجميعها ظاهرة وقابلة للتفاعل.
- نجحت اختبارات المشروع **98/98** واكتمل بناء الإنتاج بعد إصدار `admin-app-r31.js`.

## sv25 — رصد الإصدار الحي بعد نشر r31 (2026-08-17)
- المصدر المحلي لنقطة الإدارة `admin-sv24.html` يحيل صراحةً إلى `admin-app-r31.js?v=partners-handoff-r31` و`admin-partners-manager-r18.js?v=partners-r18`.
- عند التحقق من الدومين بعد النشر، حمّل المتصفح فعليًا `admin-app-r30.js?v=merge-r3` ومدير الشركاء r18، وكانت `window.WajbatPartnersManager` غير معرفة وظهر محرر القائمة التقليدي (33 زر «تعديل»). هذا دليل على أن نقطة الدخول الحية ما زالت تستجيب بإصدار أقدم رغم نجاح نقطة الحفظ، وليس فشلًا في منطق r31 المحلي.

## sv25 — تحقق حي نهائي مؤكد (الإصدار 76293396)
- بعد اكتمال انتشار الإصدار، فتحت نقطة الإدارة الحية `admin-sv24.html` بمعرّف إصدار فريد. ظهرت واجهة «مركز إدارة الشراكات» الجديدة، ثم فتح محرر جامعة طيبة بالخطوات الثلاث.
- الحقول الحية المتاحة والقابلة للتفاعل: **تغيير الشعار من الجهاز**، **رابط الموقع الرسمي** بقيمة `https://www.taibahu.edu.sa/`، و**وصف الجهة للزوار**، إلى جانب أزرار الإلغاء والحفظ.
- لم تُحفظ أي تغييرات خلال هذا الفحص؛ يثبت الاختبار أن المحرر المتخصص هو المستخدم في الإنتاج وأن المحرر التقليدي لم يعد يستبدله.

## sv26-r36 — تحقق حي بعد النشر

- بعد اكتمال انتشار checkpoint `db8f7955`، فُتحت الصفحة الرئيسية الحية على `https://uploadplus-47dkogbk.manus.space/?release=sv26-r36-db8f7955`.
- يظهر القسم الرئيسي ثم قسم **خدماتنا الأكاديمية** بست بطاقات أصلية: حل الواجبات، التكاليف الجامعية، التقارير، مشاريع التخرج، الأسايمنت، والتحليل الإحصائي، ثم قسم **لماذا تختار واجبات بلس؟** ونداء الإجراء؛ لم يُحذف أي محتوى أو خدمة.
- ظهر في أول تحميل موضع فارغ عابر أثناء تهيئة عامل التوافق، ثم اكتمل العرض تلقائيًا في الفحص التالي. السبب كان أن الصفحة المخزنة سابقًا تشير إلى r35، ويحوّل عامل التوافق المحدّث طلبها إلى r36 إلى أن يصل مدخل HTML الجديد إلى جميع طبقات التوزيع.
