(function () {
  ...
  /* v6: إخراج «downloads» من القائمة ليتولى مدير التحميلات المخصص (r10) كامل المسؤولية */
  const structuredKeys = new Set(["homePage", "design", "contact", "about", "team", "universities", "plans", "articles", "reviews", "faqs"]);
  ...
  function listWorkspace(key) {
    ...
    return `${head(key)}<div class="workspace-head structured-list-head"><p>أضف، عدّل، أظهر أو أخفِ، ثم رتب العناصر دون التعامل مع أي بيانات تقنية.</p><button class="btn btn-small" type="button" data-structured-add data-structured-key="${key}">+ إضافة</button></div><div class="workspace-body">${rows ? `<div class="item-list structured-list">${rows}</div>` : `<div class="empty">لا توجد عناصر في هذا القسم.</div>`}</div>${footer()}`;
  }
  ...
  document.addEventListener("click", event => {
    ...
    if (button.hasAttribute("data-structured-move")) { const list = currentData(key) || []; const [rawIndex, direction] = button.dataset.structuredMove.split(":"); const index = Number(rawIndex); const target = direction === "up" ? index - 1 : index + 1; if (target < 0 || target >= list.length) return; [list[index], list[target]] = [list[target], list[index]]; list.forEach((item, itemIndex) => { item.sortOrder = itemIndex; }); void saveList(key, list); return; }
  ...
  document.addEventListener("change", event => {
    ...
    void upload.then(result => {
      ...
      const isLogoTarget = /structured-logo-image|team-photo|team-logo/i.test(input.id);
      if (kind === "file") admin().toast("تم رفع الملف. احفظ التعديلات لربطه بالعنصر.");
