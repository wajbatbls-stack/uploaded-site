(function () {
  ...
  const imagePicker = (current, target) => `<div class="structured-media-picker"><select data-media-target="${target}">${selectOptions(mediaItems("image"), current, "اختر صورة من المعرض")}</select><button class="btn btn-outline btn-small" type="button" data-structured-upload="${target}">رفع صورة جديدة</button><input id="${target}" type="file" accept="image/jpeg,image/png,image/webp,image/gif,image/svg+xml" hidden /></div>`;
  const filePicker = (current, target) => `<div class="structured-media-picker"><select data-media-target="${target}">${selectOptions(mediaItems("file"), current, "اختر ملفاً من المعرض")}${current && !mediaItems("file").some(item => item.url === current) ? `<option value="${esc(current)}" selected>الملف المرتبط حالياً</option>` : ""}</select><button class="btn btn-outline btn-small" type="button" data-structured-file-upload="${target}">رفع ملف</button><input id="${target}" data-structured-upload-kind="file" type="file" accept="application/pdf,.doc,.docx,image/jpeg,image/png,image/webp" hidden /></div>`;
  ...
  const settingsPreview = (key, settings) => {
    ...
    if (key === "design") return `<aside class="structured-settings-preview structured-logo-preview" data-structured-preview="design" aria-live="polite"><span>معاينة قبل الحفظ</span><strong>شعار الموقع</strong><div class="structured-logo-preview-frame"><img data-structured-preview-logo src="${esc(settings.logoUrl || "")}" alt="معاينة شعار الموقع" ${settings.logoUrl ? "" : "hidden"} /><p data-structured-preview-placeholder ${settings.logoUrl ? "hidden" : ""}>اختر صورة من المعرض أو ارفع صورة لتظهر هنا قبل الحفظ.</p></div></aside>`;
    return "";
  };
  ...
  document.addEventListener("change", event => {
    ...
    void upload.then(result => {
      ...
      if (select) { select.insertAdjacentHTML("beforeend", `<option value="${esc(result.url)}">${esc(file.name)}</option>`); select.value = result.url; updateSettingsPreview(select.closest("form[data-structured-settings]")); }
      admin().toast(kind === "file" ? "تم رفع الملف. احفظ التعديلات لربطه بالفئة." : "تم رفع الصورة. احفظ التعديلات لتطبيقها.");
      input.value = "";
