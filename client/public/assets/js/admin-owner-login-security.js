(() => {
  ...
  async function query(procedure, input) {
    ...
    const response = await fetch(`${procedureUrl(procedure)}${suffix}`, { credentials: "same-origin" });
  ...
  async function mutate(procedure, input) {
    const response = await fetch(procedureUrl(procedure), {
      method: "POST",
  ...
  async function signInWithPasskey() {
    ...
    try {
      ...
      if (error?.name === "NotAllowedError") {
        if (errorBox) errorBox.textContent = "تم إلغاء التحقق بالبصمة أو لم تُتح وسيلة دخول مطابقة.";
        return;
      }
      if (errorBox) errorBox.textContent = messageOf(error);
    }
  }
  ...
  function passkeysPanel(passkeys) {
    ...
    return `<section class="editor owner-login-security-panel" data-owner-passkeys>
      ...
      <div class="two-col owner-passkey-form">
        <div class="field"><label>اسم هذه الوسيلة</label><input name="passkeyLabel" maxlength="120" value="جهاز المالك" /></div>
        <div class="field"><label>كلمة المرور الحالية للتأكيد</label><input name="passkeyCurrentPassword" type="password" dir="ltr" autocomplete="current-password" minlength="8" /></div>
        <div class="field field-actions"><button class="btn" type="button" data-owner-passkey-register ${supported ? "" : "disabled"}>تسجيل بصمة / Passkey لهذا الجهاز</button></div>
      </div>
  ...
  async function registerPasskey() {
    ...
    const label = String(panel?.querySelector('[name="passkeyLabel"]')?.value || "Passkey المالك").trim();
    ...
    toast("تم تسجيل وسيلة الدخول الآمنة. يمكنك استخدامها من شاشة الدخول.");
  }
  ...
  document.addEventListener("click", event => {
    ...
    if (button.matches("[data-owner-passkey-register]")) {
      event.preventDefault();
      void registerPasskey().catch(error => toast(messageOf(error)));
    }
